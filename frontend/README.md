...
The firmware is located in `sketch_jan13a.ino` and is organized into modular functional layers:
...
The workstation includes an advanced clinical osmotherapy prediction model implemented in `frontend/app.js`.
...
- **`index.html`:** Semantic, accessible HTML5 layout featuring unique IDs for automated testability and responsive medical telemetry cockpits.
- **`style.css`:** Tailored medical design system with over 3,000 lines of CSS. Includes custom glassmorphic cards, glowing status pills, responsive CSS grids, keyframe-animated telemetry beacons, and an official `@media print` layout.
- **`app.js`:** Complete event-driven application state controller managing Web Serial communication, fluid physics calculations, AI prediction updates, modal lifecycle, and doctor report formatting.
...
2. Open `sketch_jan13a.ino` in Arduino IDE.
...
| **Timer continues running after pressing Stop** | Firmware version mismatch | Ensure `sketch_jan13a.ino` v2.5 is flashed; it halts timer in `CMD:STOP`. |
